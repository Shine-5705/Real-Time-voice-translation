function createTranscriptQueueService({
  ...
  const sttContextLines = [];
  const sttContextWindow = Number(env('STT_CLEANUP_CONTEXT_SEGMENTS', '6'));

  function appendSttContextLine(text) {
    const value = String(text || '').trim();
    if (!value) return;
    sttContextLines.push(value);
    while (sttContextLines.length > sttContextWindow) sttContextLines.shift();
  }
    ...
    try {
      while (transcriptQueue.length > 0) {
        ...
        try {
          ...
          const sttOnly = Boolean(item.activeConfig?.sttOnly);
          const cleanupText = typeof cleanupTranscriptText === 'function'
            ? await cleanupTranscriptText({
              text: item.text,
              languageHint: item.detectedLanguage,
              recentContextLines: sttContextLines,
            })
            : String(item.text || '').trim();
          const correctedText = String(cleanupText || item.text || '').trim();
          appendSttContextLine(correctedText || item.text);
          ...
          event.sender.send('transcript', {
            ...
            incoming: isIncoming,
            sttElapsedMs: Number(item.sttElapsedMs || 0),
          });
          if (sttOnly) {
            if (Boolean(item.activeConfig?.injectFinalText) && typeof injectToFocusedApp === 'function') {
              const injectResult = await injectToFocusedApp(correctedText);
              if (!injectResult.ok) {
                event.sender.send('translation-status', {
                  running: true,
                  message: `Typing injection failed: ${injectResult.error || 'unknown error'}`,
                });
              }
            }
            continue;
