...
const { createCleanupService } = require('../services/stt/cleanupService');
const { createTextInjectionService } = require('../services/stt/textInjectionService');
...
function isFinalTranscriptMessage(msg = {}) {
  if (msg == null || typeof msg !== 'object') return true;
  if (typeof msg.is_final === 'boolean') return msg.is_final;
  if (typeof msg.final === 'boolean') return msg.final;
  if (typeof msg.isFinal === 'boolean') return msg.isFinal;
  if (typeof msg.kind === 'string') return msg.kind.toLowerCase() !== 'partial';
  if (typeof msg.type === 'string') {
    const t = msg.type.toLowerCase();
    if (t === 'interim' || t === 'partial') return false;
  }
  return true;
}
...
function bootRuntime({ app, createMainWindow }) {
  ...
  const cleanupService = createCleanupService({ env, logInfo, logError, fetchWithTimeout });
  const textInjectionService = createTextInjectionService({ logInfo, logError });
  ...
  const transcriptQueue = createTranscriptQueueService({
    ...
    cleanupTranscriptText: cleanupService.cleanupTranscriptText,
    injectToFocusedApp: textInjectionService.injectToFocusedApp,
  });

  function enqueueTranscript(event, msg) {
    if (!msg || !msg.text || !msg.text.trim()) return;
    const msgType = String(msg.type || 'transcript').toLowerCase();
    if (!['transcript', 'interim', 'partial'].includes(msgType)) return;
    const isFinal = isFinalTranscriptMessage(msg);
    if (!isFinal) {
      event.sender.send('transcript', {
        kind: 'partial',
        text: String(msg.text || '').trim(),
        correctedText: String(msg.text || '').trim(),
  ...
  function registerIpc() {
    ipcMain.on('start-translation', (event, config = {}) => {
      ...
      const sttOnly = Boolean(config.sttOnly);
      const injectFinalText = Boolean(config.injectFinalText);
      ...
      artifacts.writeEvent('session_start', {
        ...
        stt_only: sttOnly,
        inject_final_text: injectFinalText,
        platform,
      ...
      if (sttModePref === 'rest') {
        ...
        if (sttOnly) {
          return sendStatus(event, true, `STT-only mode started (${sourceLanguage}).`);
        }
      ...
      realtimeWs.connectSTT(event, sourceLanguage)
        .then(() => {
          state.sttMode = 'ws';
          if (sttOnly) {
            sendStatus(event, true, `STT-only mode started (${sourceLanguage}).`);
            return;
          }
