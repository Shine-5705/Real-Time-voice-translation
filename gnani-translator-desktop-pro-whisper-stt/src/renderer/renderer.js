...
const translatorTabBtn = document.getElementById('translatorTabBtn');
const sttTabBtn = document.getElementById('sttTabBtn');
const outputSection = document.getElementById('outputSection');
const platformSection = document.getElementById('platformSection');
const deliverySection = document.getElementById('deliverySection');
const bidirectionalSection = document.getElementById('bidirectionalSection');
const targetSection = document.getElementById('targetSection');
const voiceCloneSection = document.getElementById('voiceCloneSection');
const passthroughSection = document.getElementById('passthroughSection');
const sttInsights = document.getElementById('sttInsights');
const sttSegmentsCountEl = document.getElementById('sttSegmentsCount');
const sttAvgLatencyEl = document.getElementById('sttAvgLatency');
const sttLastLatencyEl = document.getElementById('sttLastLatency');
const sttTuningSection = document.getElementById('sttTuningSection');
const audioPresetSelect = document.getElementById('audioPresetSelect');
const injectTypingToggle = document.getElementById('injectTypingToggle');
...
let MIC_NOISE_GATE_THRESHOLD = 0.008;
...
const UI_MODE_TRANSLATION = 'translation';
const UI_MODE_STT = 'stt';
let activeUiMode = UI_MODE_TRANSLATION;
let sttSegmentsCount = 0;
let sttLatencyTotalMs = 0;
let sttLatencySamples = 0;
let activeAudioPreset = 'wispr_catch';
let sttPartialLineEl = null;
const AUDIO_PRESETS = {
  wispr_catch: {
    preampGain: 1.5,
    noiseGateThreshold: 0.0095,
    adaptiveSpeechDb: 11,
    adaptiveHangoverFrames: 8,
    lowpassHz: 3600,
    overlapAttenuationGain: 0.45,
  },
  noisy_room: {
    preampGain: 1.45,
    noiseGateThreshold: 0.012,
    adaptiveSpeechDb: 15,
    adaptiveHangoverFrames: 6,
    lowpassHz: 3400,
    overlapAttenuationGain: 0.38,
  },
  balanced: {
    preampGain: 1.35,
    noiseGateThreshold: 0.008,
    adaptiveSpeechDb: 12,
    adaptiveHangoverFrames: 4,
    lowpassHz: 3800,
    overlapAttenuationGain: 0.35,
  },
};
...
function clearTranscript() {
  ...
  sttPartialLineEl = null;
}

function isSttOnlyMode() {
  return activeUiMode === UI_MODE_STT;
}

function resetSttInsights() {
  sttSegmentsCount = 0;
  sttLatencyTotalMs = 0;
  sttLatencySamples = 0;
  if (sttSegmentsCountEl) sttSegmentsCountEl.textContent = '0';
  if (sttAvgLatencyEl) sttAvgLatencyEl.textContent = 'n/a';
  if (sttLastLatencyEl) sttLastLatencyEl.textContent = 'n/a';
}

function updateSttInsights(sttElapsedMs) {
  sttSegmentsCount += 1;
  if (sttSegmentsCountEl) sttSegmentsCountEl.textContent = String(sttSegmentsCount);
  const latency = Number(sttElapsedMs);
  if (Number.isFinite(latency) && latency > 0) {
    sttLatencyTotalMs += latency;
    sttLatencySamples += 1;
    if (sttLastLatencyEl) sttLastLatencyEl.textContent = `${Math.round(latency)}ms`;
    if (sttAvgLatencyEl) {
      const avg = Math.round(sttLatencyTotalMs / Math.max(1, sttLatencySamples));
      sttAvgLatencyEl.textContent = `${avg}ms`;
    }
  }
}

function applyUiMode(nextMode) {
  activeUiMode = nextMode === UI_MODE_STT ? UI_MODE_STT : UI_MODE_TRANSLATION;
  const sttMode = isSttOnlyMode();
  const hide = (node, shouldHide) => {
    if (!node) return;
    node.classList.toggle('mode-hidden', shouldHide);
  };
  hide(outputSection, sttMode);
  hide(platformSection, sttMode);
  hide(deliverySection, sttMode);
  hide(bidirectionalSection, sttMode);
  hide(targetSection, sttMode);
  hide(voiceCloneSection, sttMode);
  hide(passthroughSection, sttMode);
  hide(sttInsights, !sttMode);
  hide(sttTuningSection, !sttMode);
  if (translatorTabBtn) translatorTabBtn.classList.toggle('active', !sttMode);
  if (sttTabBtn) sttTabBtn.classList.toggle('active', sttMode);
  if (startBtn) startBtn.textContent = sttMode ? 'Start STT' : 'Start Translation';
  if (stopBtn) stopBtn.textContent = sttMode ? 'Stop STT' : 'Stop Translation';
  if (!startBtn.disabled) {
    statusEl.textContent = sttMode
      ? 'Status: Ready for Whisper-style STT test'
      : 'Status: Ready (Install virtual cable first)';
  }
  clearTranscript();
  resetSttInsights();
  appendTranscriptLine(
    sttMode ? 'STT-only tab active. Start to view corrected transcripts.' : 'Transcript will appear here...',
    'line-muted'
  );
}

function renderSttPartialLine(text) {
  const value = String(text || '').trim();
  if (!value) return;
  if (!sttPartialLineEl) {
    sttPartialLineEl = document.createElement('div');
    sttPartialLineEl.classList.add('line-muted');
    transcriptBox.appendChild(sttPartialLineEl);
  }
  sttPartialLineEl.textContent = `partial: ${value}`;
  transcriptBox.scrollTop = transcriptBox.scrollHeight;
}

function clearSttPartialLine() {
  if (!sttPartialLineEl) return;
  try {
    sttPartialLineEl.remove();
  } catch (_e) {
    // no-op
  }
  sttPartialLineEl = null;
...
function applyAudioPreset(presetId, options = {}) {
  const preset = AUDIO_PRESETS[presetId] || AUDIO_PRESETS.wispr_catch;
  activeAudioPreset = AUDIO_PRESETS[presetId] ? presetId : 'wispr_catch';
  PREAMP_GAIN = preset.preampGain;
  MIC_NOISE_GATE_THRESHOLD = preset.noiseGateThreshold;
  MIC_ADAPTIVE_SPEECH_DB = preset.adaptiveSpeechDb;
  MIC_ADAPTIVE_HANGOVER_FRAMES = preset.adaptiveHangoverFrames;
  MIC_LOWPASS_HZ = preset.lowpassHz;
  OVERLAP_ATTENUATION_GAIN = preset.overlapAttenuationGain;
  adaptiveNoiseFloor = 0.002;
  adaptiveHangoverLeft = 0;
  if (lowpassNode) lowpassNode.frequency.value = MIC_LOWPASS_HZ;
  if (returnLowpassNode) returnLowpassNode.frequency.value = MIC_LOWPASS_HZ;
  if (audioPresetSelect && audioPresetSelect.value !== activeAudioPreset) {
    audioPresetSelect.value = activeAudioPreset;
  }
  if (options.announce) {
    appendTranscriptLine(`STT tuning preset: ${activeAudioPreset}`, 'line-muted');
  }
}
...
function validateLanguages(sttOnly = false) {
  if (!sourceLangSelect.value) {
    statusEl.textContent = 'Status: Select source language';
    return false;
  }
  if (sttOnly) {
    return true;
  ...
  if (!targetLangSelect.value) {
    statusEl.textContent = 'Status: Select target language';
    return false;
  }
...
startBtn.addEventListener('click', () => {
  const sttOnly = isSttOnlyMode();
  const selectedMic = micSelect.value;
  const selectedOutput = sttOnly ? '' : outputSelect.value;
  const platform = selectedPlatform();
  const deliveryMode = selectedDeliveryMode();
  if (!selectedMic || (!sttOnly && !selectedOutput)) {
    statusEl.textContent = sttOnly
      ? 'Status: Select microphone'
      : 'Status: Select both microphone and output device';
    return;
  }
  ...
  const bidir = !sttOnly && Boolean(bidirectionalToggle && bidirectionalToggle.checked);
  ...
  startRealtimePipeline(selectedMic, selectedOutput, {
    ...
    .then(() => {
      window.electronAPI.startTranslation({
        ...
        targetLanguage: sttOnly ? sourceLangSelect.value : targetLangSelect.value,
        ...
        sttOnly,
        injectFinalText: sttOnly && injectTypingToggle && injectTypingToggle.checked,
      });
      ...
      if (sttOnly) {
        statusEl.textContent = 'Status: Whisper-style STT running (autocorrected transcript only).';
        return;
      }
      if (platform === 'genesys') {
...
window.electronAPI.onTranscript((payload) => {
  if (!payload || typeof payload !== 'object') return;
  if (isSttOnlyMode()) {
    if (payload.translated || payload.incoming) return;
    if (String(payload.kind || '').toLowerCase() === 'partial') {
      renderSttPartialLine(payload.correctedText || payload.text);
      return;
    }
    clearSttPartialLine();
    const corrected = String(payload.correctedText || payload.text || '').trim();
    if (!corrected) return;
    const spk = payload.speakerId ? ` [${payload.speakerId}]` : '';
    appendTranscriptLine(`STT${spk}: ${corrected}`);
    const raw = String(payload.text || '').trim();
    if (raw && raw !== corrected) {
      appendTranscriptLine(`raw: ${raw}`, 'line-muted');
    }
    updateSttInsights(payload.sttElapsedMs);
    return;
  }
...
if (translatorTabBtn) {
  translatorTabBtn.addEventListener('click', () => applyUiMode(UI_MODE_TRANSLATION));
}
if (sttTabBtn) {
  sttTabBtn.addEventListener('click', () => applyUiMode(UI_MODE_STT));
}
if (audioPresetSelect) {
  audioPresetSelect.addEventListener('change', () => {
    applyAudioPreset(audioPresetSelect.value, { announce: true });
  });
}

buildLanguageOptions();
loadDevices();
applyAudioPreset('wispr_catch');
applyUiMode(UI_MODE_TRANSLATION);
